    ...
    >
      ...
      <div className="relative mt-2">
        <Headless.ComboboxInput
          className="block w-full rounded-md bg-zinc-900 py-1.5 pl-3 pr-12 text-base text-zinc-200 outline-hidden border-zinc-700 placeholder:text-zinc-500 focus:border-primary-600 sm:text-sm/6 focus:ring-0 focus:outline-0"
          placeholder={placeholder}
          onBlur={() => setQuery('')}
          onChange={(e) => setQuery(e.target.value)}
          displayValue={getDisplayValue}
        />
        <Headless.ComboboxButton className="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-hidden">
          <ChevronUpDownIcon className="h-5 w-5 text-zinc-600" aria-hidden="true" />
        </Headless.ComboboxButton>

        {filteredItems.length > 0 && (
          <Headless.ComboboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-zinc-900 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-hidden sm:text-sm">
