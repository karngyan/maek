...
func TestList(t *testing.T) {
	...
	_, err := notes.CreateNoteCtx(context.Background(), notes.WithContent("{ \"foo\": \"bar\" }"), notes.WithWorkspace(cs.Workspace), notes.WithCreatedBy(cs.User), notes.WithFavorite(true))
	assert.Nil(t, err)
